...
async def send_offer(websocket):
    ...
    def on_dtls_state_change():
        ...
        logging.info(f"DTLS state changed: {state}")
        if state == "failed":
            logging.error("DTLS handshake failed.")

    # Create an SDP offer and retry if necessary
    retries = 5
    backoff = 1
    ...
    def on_icecandidate(event):
        if event.candidate:
            ...
            logging.debug(f"New ICE candidate: {candidate_info}")
            asyncio.ensure_future(websocket.send(str(candidate_info)))  # Send ICE candidates via WebSocket
