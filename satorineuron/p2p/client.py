...
class MessageClient:
    ...
    def update_message_stats(self, peer_id: str, timestamp: float) -> bool:
        ...
        with self.lock:
            ...
            warning_limit = self.rate_limit_config.message_limit * \
                self.rate_limit_config.warning_threshold
        ...
        with self.lock:
            ...
            for msg in self.message_history:
                if peer_id is None or msg['from'] == peer_id:
                    print(
                        f"[{msg['received_at']}] From {msg['from']}: {msg['message']}")
            ...
            if data and data.get('messages'):
                for msg in data['messages']:
                    ...
                    if self.update_message_stats(peer_id, timestamp):
                        ...
                        print(
                            f"Disconnected from peer {peer_id} due to rate limiting")
...
if __name__ == "__main__":
    ...
    print(
        f"- Maximum {client.rate_limit_config.message_limit} messages per {client.rate_limit_config.window_size} seconds")
    print(
        f"- Warning at {client.rate_limit_config.warning_threshold * 100}% of limit")
            ...
            if choice == "1":
                ...
                for peer_id, last_seen in peers.items():
                    if peer_id != CLIENT_ID:
                        print(
                            f"Peer ID: {peer_id}, Last seen: {time.ctime(last_seen)}")
                ...
                peer_id = input(
                    "Enter peer ID to filter (or press Enter for all): ").strip()
                client.show_message_history(peer_id if peer_id else None)

            elif choice == "7":
                peer_id = input(
                    "Enter peer ID to check (or press Enter for all): ").strip()
                ...
                if peer_id and stats:
                    ...
                    print(
                        f"- Messages in current window: {stats['messages_in_window']}/{stats['rate_limit']}")
                    ...
                    for peer, peer_stats in stats.items():
                        print(f"\nPeer {peer}:")
                        print(
                            f"- Total messages: {peer_stats['total_messages']}")
                        print(
                            f"- Messages in current window: {peer_stats['messages_in_window']}/{peer_stats['rate_limit']}")
